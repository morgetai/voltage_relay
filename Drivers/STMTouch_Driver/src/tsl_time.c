...
void TSL_tim_ProcessIT(void)
{
  ...
  if (count_1s > (TSLPRM_TICK_FREQ - 1))
  {
    ...
    if (TSL_Globals.Tick_sec > 59)  // Due to DTO counter on 6 bits...
    {
      TSL_Globals.Tick_sec = 0;
			TSL_Globals.Tick_min ++;
			if (TSL_Globals.Tick_min > 59)
				TSL_Globals.Tick_min = 0;
    }
    count_1s = 0;
		Global_Tick.tm_sec = TSL_Globals.Tick_sec;
		Global_Tick.tm_min = TSL_Globals.Tick_min;
  }
